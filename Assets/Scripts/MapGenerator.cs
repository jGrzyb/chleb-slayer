...
public class MapGenerator : MonoBehaviour
{
    ...
    [Tooltip("Szansa (w %) na pojawienie si� dekoracji na pustym polu")]
    [Range(0f, 100f)]
    ...
    [Header("�ciany - Kraw�dzie (G�rny rz�d)")]
    public TileBase[] wallTopTiles;
    public TileBase wallBottom;
    public TileBase wallLeft;
    public TileBase wallRight;

    // --- NOWE: Dolny rz�d g�rnej �ciany ---
    [Header("�ciany - G�rna (Dolny rz�d)")]
    [Tooltip("Te kafelki pojawi� si� bezpo�rednio pod g�rn� kraw�dzi� �ciany")]
    ...
    [Header("Gotowe szablony przeszk�d")]
    ...
    [Tooltip("Odleg�o�� spawnera od �ciany w kafelkach. 1 oznacza kratk� tu� obok �ciany.")]
    ...
    public void GenerateObstacles()
    {
        ...
        while (obstaclesPlaced < numberOfObstacles && attempts < maxAttempts)
        {
            ...
            // --- ZMIANA: G�rna granica spawnowania przeszk�d obni�ona o 1 (-2 zamiast -1) ---
            ...
            if (isHorizontal)
            {
                // --- ZMIANA: G�rna �ciana zaczyna si� od mapPositionY + height - 2 ---
            ...
            for (int cx = checkMinX; cx <= checkMaxX && canPlace; cx++)
            {
                for (int cy = checkMinY; cy <= checkMaxY; cy++)
                {
                    ...
                    if (obstacleTilemap.HasTile(pos))
                    {
                        ...
                        // --- ZMIANA: G�rna �ciana zaczyna si� od wysoko�ci height - 2 ---
            ...
            for (int cx = checkMinX; cx <= checkMaxX && canPlace; cx++)
            {
                for (int cy = checkMinY; cy <= checkMaxY; cy++)
                {
                    ...
                    if (obstacleTilemap.HasTile(pos))
                    {
                        ...
                        bool isLeftOrRightWall = (localCx <= 0 || localCx >= width - 1);
                        // --- ZMIANA: G�rna �ciana zajmuje height - 1 oraz height - 2 ---
                        bool isTopOrBottomWall = (localCy <= 0 || localCy >= height - 2);
                        ...
                        if (isOuterWall)
                        {
                            ...
                            else
                            {
                                ...
                                // --- ZMIANA: Wewn�trzna granica dla g�rnej �ciany to teraz height - 3 ---
        ...
        {
            Debug.LogWarning($"Wygenerowano {obstaclesPlaced}/{numberOfObstacles} przeszk�d. Brakuje miejsca!");
        }
    }
                ...
                if (isLeft || isRight || isBottom || isTop)
                {
                    ...
                    else if (isTop)
                    {
                        if (wallTopTiles != null && wallTopTiles.Length > 0)
                        {
                            ...
                            // --- NOWE: Stawiamy doln� cz�� g�rnej �ciany ---
    ...
    void SpawnEnemySpawners()
    {
        ...
        {
            Debug.LogError("Mapa jest za ma�a, aby umie�ci� spawnery w takiej odleg�o�ci od �ciany!");
            return;
        }
        ...
        // --- ZMIANA: G�rna granica uwzgl�dnia grubsz� o 1 kratk� �cian� (-2 zamiast -1) ---
    ...
    void GenerateDecorations()
    {
        ...
        {
            // --- ZMIANA: Dekoracje omijaj� doln� cz�� g�rnej �ciany (height - 2) ---
    ...
    void GenerateCameraBounds()
    {
        ...
        if (foundConfiner != null)
        {
            ...
            Debug.Log("<color=green>Sukces!</color> Confiner znalaz� map� o rozmiarze " + width + "x" + height);
        }
        else
        {
